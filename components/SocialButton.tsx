...
const SocialButton: React.FC<SocialButtonProps> = ({ link }) => {
  return (
    <a
      ...
      className="group relative flex items-center w-full px-6 py-5 rounded-[40px] bg-[#0c0c0c] border border-white/[0.04] shadow-[0_10px_30px_-15px_rgba(0,0,0,0.5)] transition-all duration-500 [transition-timing-function:cubic-bezier(0.23,1,0.32,1)] hover:-translate-y-1 hover:bg-[#111111] hover:border-white/20 active:translate-y-0.5 active:scale-[0.98] active:bg-[#080808] overflow-hidden select-none cursor-pointer"
    ...
    >
      ...
      {/* Content Container - Gap increased to 24px (gap-6) to move text slightly further from icon while remaining left-aligned */}
      <div className="flex items-center w-full relative z-10 gap-6">
        ...
        {/* Label - Text remains left-aligned and refined */}
        <span className="flex-grow text-left text-[12px] font-medium tracking-[0.2em] text-zinc-400 uppercase transition-all duration-500 group-hover:text-white group-hover:tracking-[0.24em]">
          {link.name}
        </span>
